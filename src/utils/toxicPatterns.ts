import { ToxicPattern } from '../types/index.js';

// Kata-kata toxic / kasar dalam Bahasa Indonesia dan Inggris
export const TOXIC_WORDS_ID = [
  // Makian umum
...
// Pola spam invite
export const INVITE_PATTERN = /discord\.gg\/[a-zA-Z0-9]+|discord\.com\/invite\/[a-zA-Z0-9]+/gi;

// Pola URL umum
export const URL_PATTERN = /https?:\/\/[^\s]+/gi;

// Fungsi pengecekan toxic
export function containsToxicWord(text: string): { found: boolean; word: string; category: string } {
  const lower = text.toLowerCase().replace(/\s+/g, ' ');

  for (const word of TOXIC_WORDS_ID) {
  ...
  for (const word of TOXIC_WORDS_EN) {
    const pattern = new RegExp(`\\b${word.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'gi');
    if (pattern.test(lower)) {
...
// Fungsi cek phishing
export function isPhishingLink(text: string): { isPhishing: boolean; url: string; reason: string } {
