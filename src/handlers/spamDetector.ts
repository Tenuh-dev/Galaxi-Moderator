...
import { SpamTracker } from '../types/index.js';
import { getGuildSettings } from '../utils/database.js';

const spamMap = new Map<string, SpamTracker>();

export async function detectSpam(message: Message): Promise<{ isSpam: boolean; type: string }> {
  if (!message.guild || !message.member) return { isSpam: false, type: '' };

  const settings = getGuildSettings(message.guild.id);
  const spamThreshold = settings.spam_threshold ?? config.moderation.spamThreshold;
  const spamInterval = settings.spam_interval_ms ?? config.moderation.spamIntervalMs;
  const maxMentions = settings.max_mentions ?? config.moderation.maxMentions;
  ...
  const isDuplicate = recentMessages.filter(m =>
    m.author.id === message.author.id &&
    m.content === message.content &&
    m.id !== message.id &&
    now - m.createdTimestamp < spamInterval
  ).length >= 2;
  ...
  setTimeout(() => {
    if (spamMap.has(key)) {
      ...
      if (t.messages.length === 0) spamMap.delete(key);
    }
  }, spamInterval);
...
async function getRecentMessages(message: Message): Promise<Message[]> {
  try {
...
export async function handleSpam(message: Message, type: string): Promise<void> {
  ...
  try {
    ...
    if (message.channel instanceof TextChannel) {
      const warning = await message.channel.send({
        content: `${config.emojis.spam} <@${message.author.id}> **Peringatan!** Kamu terdeteksi melakukan **${spamTypeLabel}**. Pesan dihapus dan kamu di-timeout selama 5 menit.`,
      });
