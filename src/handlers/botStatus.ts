import { Message, EmbedBuilder, Guild, GuildMember } from 'discord.js';
import { config } from '../config/index.js';
import os from 'os';

export async function handleBotStatusCommand(message: Message): Promise<void> {
  ...
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .setTitle(`${config.emojis.bot} Status Bot di Server ${guild.name}`)
      .setThumbnail(client.user?.displayAvatarURL() || null)
      .setDescription(`Berikut adalah daftar semua bot yang aktif di server ini:\n\u200b`)
      .addFields(
        {
          name: `${config.emojis.star} Galaxi Auto Moderator`,
          value: [
            `> **Status:** 🟢 Online`,
            `> **Ping:** ${ping}ms`,
            `> **Uptime:** ${uptime}`,
    ...
    if (otherBots) {
      embed.addFields({
        name: `${config.emojis.bot} Bot Lain di Server (${bots.size - 1})`,
...
function getBotStatus(member: GuildMember): string {
